...
from src.SEDR_model import SEDR,SEDR_GAT,SEDR_GIN,SEDR_SAGE,SEDR1,SEDR_Transformer,SEDR_gcn_cluster,SEDR_GATv2,SEDR_GAT_topic
import scanpy as sc
import pandas as pd
...
class SEDR_Train:
    def __init__(self, node_X, graph_dict,data, params):
        ...
        self.model = SEDR_GATv2(self.params.cell_feat_dim, self.params).to(self.device)
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            latent_z, mu, logvar, de_feat, _, feat_x, _ = self.model(self.node_X, self.adj_norm,training)
    ...
    def process(self):
        self.model.eval()
        training=False
        latent_z, _, _, _, q, feat_x, gnn_z = self.model(self.node_X, self.adj_norm,training)
    ...
    def train_with_dec(self):
        ...
        for epoch_id in range(self.epochs):
            ...
            self.optimizer.zero_grad()
            latent_z, mu, logvar, de_feat, out_q, _, _ = self.model(self.node_X, self.adj_norm,training)
            loss_gcn = gcn_loss(preds=self.model.dc(latent_z), labels=self.adj_label, mu=mu,
                                logvar=logvar, n_nodes=self.params.cell_num, norm=self.norm_value, mask=self.adj_label)
            loss_rec = reconstruction_loss(de_feat, self.node_X)
            # clustering KL loss
