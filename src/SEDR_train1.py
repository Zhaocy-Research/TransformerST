...
from src.SEDR_model1 import SEDR
...
class SEDR_Train:
    def __init__(self, node_X, graph_dict, label,params):
        self.params = params
        ...
        if params.using_mask is True:
            ...
            self.adj_mask = None
        self.label=torch.FloatTensor(label).squeeze(1).to(self.device)
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            latent_z, mu, logvar, de_feat, _, feat_x, _,out = self.model(self.node_X, self.adj_norm)
            ...
            # print(out.shape,self.label.shape)
            loss = self.params.feat_w * loss_rec + self.params.gcn_w * loss_gcn+self.loss_classification(out,self.label.long())
            loss.backward()
            self.optimizer.step()
    ...
    def process(self):
        self.model.eval()
        latent_z, _, _, _, q, feat_x, gnn_z,_ = self.model(self.node_X, self.adj_norm)
    ...
    def train_with_dec(self):
        ...
        bgm = BayesianGaussianMixture(n_components=10, random_state=42).fit(test_z)
        ...
        for epoch_id in range(self.epochs):
            ...
            self.optimizer.zero_grad()
            latent_z, mu, logvar, de_feat, out_q, _, _,out = self.model(self.node_X, self.adj_norm)
            loss_gcn = gcn_loss(preds=self.model.dc(latent_z), labels=self.adj_label, mu=mu,
                                logvar=logvar, n_nodes=self.params.cell_num, norm=self.norm_value, mask=self.adj_label)
            loss_rec = reconstruction_loss(de_feat, self.node_X)
            # clustering KL loss
            loss_kl = F.kl_div(out_q.log(), torch.tensor(tmp_p).to(self.device)).to(self.device)
            loss = self.params.gcn_w * loss_gcn + self.params.dec_kl_w * loss_kl + self.params.feat_w * loss_rec+1.5*self.loss_classification(out,self.label.long())
            loss.backward()
            self.optimizer.step()
