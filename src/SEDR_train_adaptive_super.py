...
from src.SEDR_model import SEDR,SEDR_GAT,SEDR_GIN,SEDR_SAGE,SEDR1,SEDR_Transformer,SEDR_gcn_cluster,SEDR_GATv2,SEDR_GAT_topic,SEDR_GATv2_adaptive,SEDR_Transformer_adaptive,SEDR_GAT_adaptive,SEDR_Transformer_adaptive_super_gai
import scanpy as sc
import pandas as pd
...
class SEDR_Train:
    def __init__(self, node_X,neighbor_x, graph_dict,data,graph_dict_prue,data_prue,spatial, params):
        self.params = params
        ...
        self.model = SEDR_Transformer_adaptive_super_gai(self.params.cell_feat_dim, self.params).to(self.device)
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            latent_z, de_feat, _, feat_x,_,_ ,_= self.model(self.node_X, self.neighbor_x,self.adj_norm,self.adj_norm_prue,self.spatial,training)
    ...
    def process(self):
        self.model.eval()
        training=False
        latent_z, _, q, feat_x,x_hr,hr_z,atten = self.model(self.node_X, self.neighbor_x,self.adj_norm,self.adj_norm_prue,self.spatial,training)
        ...
        return latent_z, q, feat_x,x_hr,hr_z,atten
    ...
    def train_with_dec(self):
        ...
        test_z, _, _ ,_,test_z1,_= self.process()
