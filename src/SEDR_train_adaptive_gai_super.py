...
from src.SEDR_model import SEDR,SEDR_GAT,SEDR_GIN,SEDR_SAGE,SEDR1,SEDR_Transformer,SEDR_gcn_cluster,SEDR_GATv2,SEDR_GAT_topic,SEDR_GATv2_adaptive,SEDR_Transformer_adaptive,SEDR_GAT_adaptive,SEDR_Transformer_adaptive_super_gai_new
import scanpy as sc
import pandas as pd
...
class SEDR_Train_new:
    def __init__(self, node_X, enhanced_weights,enhanced_index,graph_dict,data,graph_dict_prue,data_prue,spatial, params):
        self.params = params
        self.training=False
        self.device = params.device
        self.epochs = params.epochs
        self.node_X = torch.FloatTensor(node_X.copy()).to(self.device)
        self.enhanced_weights = torch.FloatTensor(enhanced_weights.copy()).to(self.device)
        self.enhanced_index= torch.FloatTensor(enhanced_index.copy()).long().to(self.device)
        ...
        self.model = SEDR_Transformer_adaptive_super_gai_new(self.params.cell_feat_dim, self.params).to(self.device)
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            latent_z, de_feat, _, feat_x,_ = self.model(self.node_X, self.enhanced_weights,self.enhanced_index,self.adj_norm,self.adj_norm_prue,self.training)
    ...
    def process(self):
        self.model.eval()
        # training=True
        latent_z, _, q, feat_x,z_super = self.model(self.node_X, self.enhanced_weights,self.enhanced_index,self.adj_norm,self.adj_norm_prue,self.training)
        latent_z = latent_z.data.cpu().numpy()
        z_super = z_super.data.cpu().numpy()
        q = q.data.cpu().numpy()
        feat_x = feat_x.data.cpu().numpy()
        # gnn_z = gnn_z.data.cpu().numpy()
        return latent_z, q, feat_x,z_super

    def train_with_dec(self):
        n_neighbors = 10
        res = 0.4
        # training = False
        if self.params.super==False:
        ...
        # self.model.cluster_layer.data = torch.tensor(bgm.means_).float().to(self.device)
            ...
            # training model
                ...
                self.optimizer.zero_grad()
                latent_z, de_feat, out_q, _ ,_= self.model(self.node_X, self.enhanced_weights,self.enhanced_index,self.adj_norm,self.adj_norm_prue,self.training)
                loss_gcn = gcn_loss_attention(preds=self.model.dc(latent_z), labels=self.adj_label, norm=self.norm_value, mask=self.adj_label)
                loss_rec = reconstruction_loss(de_feat, self.node_X)
            # clustering KL loss
            ...
            for epoch_id in range(self.epochs):
                ...
                torch.set_grad_enabled(True)
                self.optimizer.zero_grad()
                latent_z, de_feat, out_q, _, _ = self.model(self.node_X, self.enhanced_weights, self.enhanced_index,
                                                            self.adj_norm, self.adj_norm_prue, self.training)
                loss_gcn = gcn_loss_attention(preds=self.model.dc(latent_z), labels=self.adj_label,
                                              norm=self.norm_value, mask=self.adj_label)
                loss_rec = reconstruction_loss(de_feat, self.node_X)
                # clustering KL loss
                loss_kl = F.kl_div(out_q.log(), torch.tensor(tmp_p).to(self.device)).to(self.device)
                loss = self.params.gcn_w * loss_gcn + self.params.dec_kl_w * loss_kl + self.params.feat_w * loss_rec
