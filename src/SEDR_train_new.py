...
from src.SEDR_model import SEDR,SEDR_GAT,SEDR_GIN,SEDR_SAGE,SEDR1,SEDR_Transformer,SEDR_gcn_cluster,SEDR_GATv2,SEDR_GAT_topic,SEDR_GATv2_basis
import scanpy as sc
import pandas as pd
from sklearn.mixture import BayesianGaussianMixture
from sklearn.mixture import GaussianMixture
from src.helpers import KL_standard_normal
...
class SEDR_Train:
    def __init__(self, node_X, graph_dict,data, params):
        self.params = params
        self.device = params.device
        self.epochs = params.epochs
        self.node_X = torch.FloatTensor(node_X.copy()).to(self.device)
        params.x0init=torch.mean(self.node_X.detach().cpu(),0,True).numpy()
        ...
        if params.using_mask is True:
            ...
            self.adj_mask = None
        self.params.x0init = torch.mean(self.node_X.detach().cpu(), 0, True).numpy()
        self.model = SEDR_GATv2_basis(self.params.cell_feat_dim, self.params).to(self.device)
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            latent_z, mu, logvar, de_feat, _, feat_x, _,dropout_prob_logit, theta,mu_z,sigma_z= self.model(self.node_X, self.adj_norm,training)
            loss_rec=self.model.decoder.loss(self.node_X, de_feat, dropout_prob_logit, theta, self.batch_scale, self.gamma)
            # print(loss_rec)
        # latent space loss
        #     VAE_KL_loss = self.batch_scale * KL_standard_normal(mu_z, sigma_z)
            total_loss = loss_rec
            # total_loss = loss_rec + self.beta * VAE_KL_loss
            ...
            loss = self.params.feat_w * total_loss + self.params.gcn_w * loss_gcn
            loss.backward()
            self.optimizer.step()
    ...
    def process(self):
        self.model.eval()
        training=False
        latent_z, _, _, _, q, feat_x, gnn_z,_,_,_,_ = self.model(self.node_X, self.adj_norm,training)
    ...
    def train_with_dec(self):
        ...
        for epoch_id in range(self.epochs*2):
            ...
            self.optimizer.zero_grad()
            latent_z, mu, logvar, de_feat, out_q, _, _,dropout_prob_logit, theta,mu_z,sigma_z = self.model(self.node_X, self.adj_norm,training)
            loss_gcn = gcn_loss(preds=self.model.dc(latent_z), labels=self.adj_label, mu=mu,
                                logvar=logvar, n_nodes=self.params.cell_num, norm=self.norm_value, mask=self.adj_label)
            # loss_rec = reconstruction_loss(de_feat, self.node_X)
            loss_rec = self.model.decoder.loss(self.node_X, de_feat, dropout_prob_logit, theta, self.batch_scale,
                                               self.gamma)
            ...
            loss = self.params.gcn_w * loss_gcn + self.params.dec_kl_w * loss_kl + self.params.feat_w * total_loss
            loss.backward()
            self.optimizer.step()
